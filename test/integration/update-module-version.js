...
describe("UpdateModuleVersion", ()=>{
  ...
  describe("when not all params are present", ()=> {
    beforeEach(()=>{
      platform.writeTextFileSync(config.getManifestPath(), `{"rolloutPct": 10, "modules": [{"name": "test","version": "2017.10.12.04.45", "url":"https://storage.googleapis.com/install-versions.risevision.com/beta/test-lnx-32.sh"}]}`);
    });
    afterEach(()=>{
    ...
    it("does not update manifest if module name empty", (done) => {
      execFile("node", [filePath, config.getManifestPath(), "", "2017.10.13.21.21", 25], () => {
        assert.deepEqual(config.getManifest(), {rolloutPct: 10, modules: [{name: "test",version: "2017.10.12.04.45", url: "https://storage.googleapis.com/install-versions.risevision.com/beta/test-lnx-32.sh"}]});
        done();
      });
    });
    ...
    it("updates manifest when pct is not present", (done)=>{
      execFile("node", [filePath, config.getManifestPath(), "test", "2017.10.13.21.21"], (error) => {
        assert.deepEqual(config.getManifest(), {rolloutPct: 10, modules: [{name: "test",version: "2017.10.13.21.21", url: "https://storage.googleapis.com/install-versions.risevision.com/beta/test-lnx-32.sh"}]});
        done();
      });
  ...
  describe("when all params are present", ()=>{
    ...
    it("updates module manifeset", (done)=>{
      execFile("node", [filePath, config.getManifestPath(), "test", "2017.10.13.21.21", 25], (error) => {
        console.log(error);
        assert.deepEqual(config.getManifest(), {rolloutPct: 25, modules: [{name: "test",version: "2017.10.13.21.21", url: "https://storage.googleapis.com/install-versions.risevision.com/beta/test-lnx-32.sh"}]});
        done();
      });
    ...
    it("does not update specific module version if module name doesn't match", (done) => {
      execFile("node", [filePath, config.getManifestPath(), "testabc", "2017.10.13.21.21", 25], () => {
        assert.deepEqual(config.getManifest(), {rolloutPct: 25, modules: [{name: "test",version: "2017.10.12.04.45", url: "https://storage.googleapis.com/install-versions.risevision.com/beta/test-lnx-32.sh"}]});
        done();
      });
    });
  });
